...
class ImprovedAIImageAnalyzer:
    ...
    def analyze_construction_image(self, uploaded_file, room_name: str = "", room_type_hint: str = ""):
        ...
        try:
            ...
            if "openings_summary" in final_result:
                ...
                logger.info(f"Final counts - Doors: {summary.get('total_doors', 0)}, Windows: {summary.get('total_windows', 0)}, Open Areas: {summary.get('total_open_areas', 0)}")
    ...
    def _extract_text_with_verification(self, base64_image: str) -> Dict:
        ...
        **CRITICAL: Respond ONLY in valid JSON format.**
        ...
        
        {
            ...
            "measurements_found": {
                "floor_area_sf": number_or_null,
                "wall_area_sf": number_or_null,
                "ceiling_area_sf": number_or_null,
                "perimeter_lf": number_or_null,
                "ceiling_perimeter_lf": number_or_null,
                "floor_perimeter_lf": number_or_null
            },
            "openings_found": [
                {
        ...
        **CRITICAL: Respond ONLY in valid JSON format.**
        ...
        {{
            ...
            "openings_summary": {{
                ...
                "window_area_total_sf": total_window_area,
            ...
            "calculated_materials": {{
                "baseboard_length_lf": calculated_baseboard_length,
                "crown_molding_length_lf": calculated_crown_molding_length,
                "flooring_area_sf": flooring_area,
                "wall_paint_area_sf": wall_paint_area,
                "ceiling_paint_area_sf": ceiling_paint_area
            }},
            "confidence_level": "high/medium/low",
    ...
    def _cross_validate_and_correct(self, text_analysis: Dict, structured_analysis: Dict) -> Dict:
        ...
        if "verification" in text_analysis:
            ...
            logger.info(f"Structured analysis: doors={struct_doors}, windows={struct_windows}, open_areas={struct_opens}")
    ...
    def _remove_duplicate_openings(self, result: Dict):
        ...
        if len(unique_openings) != len(detailed_openings):
            ...
            interior_doors = sum(1 for o in unique_openings if o.get("type") == "Door" and not o.get("is_exterior", False))
            ...
            logger.info(f"After deduplication - Doors: {door_count}, Windows: {window_count}, Open Areas: {open_area_count}")
                ...
                for i in range(len(detailed_openings), len(text_openings)):
                    ...
                    if "Window" in opening_type:
                        ...
                        default_opening = {
                            ...
                            "area_sf": 15.0 if "Goes to Floor" in opening_type else 10.0,
    ...
    def _ensure_floor_perimeter(self, result: Dict):
        ...
        if "floor_perimeter_lf" not in extracted_dims or extracted_dims["floor_perimeter_lf"] <= 0:
            # perimeter_lf나 ceiling_perimeter_lf 사용
            floor_perimeter = extracted_dims.get("perimeter_lf", 0)
            if floor_perimeter <= 0:
                floor_perimeter = extracted_dims.get("ceiling_perimeter_lf", 0)
            
            extracted_dims["floor_perimeter_lf"] = floor_perimeter
            logger.info(f"Added floor_perimeter_lf: {floor_perimeter}")
            
            # room_geometry에도 추가
            if "room_geometry" in result:
                result["room_geometry"]["floor_perimeter_lf"] = floor_perimeter
    ...
    def _recalculate_baseboard(self, result: Dict):
        ...
        try:
            ...
            floor_perimeter = extracted_dims.get("floor_perimeter_lf", 0)
            if floor_perimeter <= 0:
                floor_perimeter = extracted_dims.get("perimeter_lf", 0)
            if floor_perimeter <= 0:
                floor_perimeter = extracted_dims.get("ceiling_perimeter_lf", 0)
    ...
    def _sanitize_result(self, result):
        ...
        if "extracted_dimensions" in result:
            ...
            dims["floor_area_sf"] = safe_float(dims.get("floor_area_sf"), 0.0)
            dims["room_area_sf"] = safe_float(dims.get("room_area_sf"), 0.0)
            dims["wall_area_sf"] = safe_float(dims.get("wall_area_sf"), 0.0)
            dims["ceiling_area_sf"] = safe_float(dims.get("ceiling_area_sf"), 0.0)
            dims["perimeter_lf"] = safe_float(dims.get("perimeter_lf"), 0.0)
            dims["ceiling_perimeter_lf"] = safe_float(dims.get("ceiling_perimeter_lf"), 0.0)
            dims["floor_perimeter_lf"] = safe_float(dims.get("floor_perimeter_lf"), 0.0)
        
        # room_geometry 처리
        if "room_geometry" in result:
            geo = result["room_geometry"]
            geo["total_floor_area_sf"] = safe_float(geo.get("total_floor_area_sf"), 0.0)
            geo["ceiling_height_ft"] = safe_float(geo.get("ceiling_height_ft"), 0.0)
            geo["total_perimeter_lf"] = safe_float(geo.get("total_perimeter_lf"), 0.0)
            geo["floor_perimeter_lf"] = safe_float(geo.get("floor_perimeter_lf"), 0.0)
        ...
        if "openings_summary" in result:
            ...
            openings["window_area_total_sf"] = safe_float(openings.get("window_area_total_sf"), 0.0)
        ...
        if "calculated_materials" in result:
            ...
            materials["baseboard_length_lf"] = safe_float(materials.get("baseboard_length_lf"), 0.0)
            materials["crown_molding_length_lf"] = safe_float(materials.get("crown_molding_length_lf"), 0.0)
            materials["flooring_area_sf"] = safe_float(materials.get("flooring_area_sf"), 0.0)
            materials["wall_paint_area_sf"] = safe_float(materials.get("wall_paint_area_sf"), 0.0)
            materials["ceiling_paint_area_sf"] = safe_float(materials.get("ceiling_paint_area_sf"), 0.0)
    ...
    def _get_default_result(self):
        ...
        return {
            ...
            "room_geometry": {
                "total_floor_area_sf": 0.0,
                "ceiling_height_ft": 0.0,
                "total_perimeter_lf": 0.0,
                "floor_perimeter_lf": 0.0
            },
            "openings_summary": {
                "total_doors": 0,
                ...
                "window_area_total_sf": 0.0,
                "open_area_width_total_ft": 0.0
            },
            "detailed_openings": [],
            "calculated_materials": {
                "baseboard_length_lf": 0.0,
                "crown_molding_length_lf": 0.0,
                "flooring_area_sf": 0.0,
                "wall_paint_area_sf": 0.0,
                "ceiling_paint_area_sf": 0.0
            },
            "confidence_level": "failed",
