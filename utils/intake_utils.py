...
def create_continuity_zone(zone_type: str, primary_room: str, connected_rooms: List[str], reason: str) -> Dict[str,
    Any]:
...
def calculate_room_quantities(room_data: Dict[str, Any], project_standards: Dict[str, Any]) -> Dict[str, float]:
    ...
    if work_scope.get("flooring", {}).get("required", False):
        ...
        quantities["flooring_install_s"] = floor_area * (1 + waste_factor)
    ...
    if work_scope.get("drywall", {}).get("required", False):
        ...
        if drywall_extent == "full_room":
            quantities["drywall_install_s"] = wall_area_net * 1.10
            quantities["drywall_tape_s"] = wall_area_net * 1.10
        else:
            # Partial drywall
            partial_area = wall_area_net * 0.3
            quantities["drywall_install_s"] = partial_area * 1.10
            quantities["drywall_tape_l"] = perimeter * 0.5 * 1.10

    if work_scope.get("trim_baseboard", {}).get("required", False):
        quantities["baseboard_install_l"] = baseboard_length_net * 1.10

        # Quarter round if applicable
        if project_standards.get("quarter_round", True):
            quantities["quarter_round_install_l"] = baseboard_length_net * 1.10
    ...
    if work_scope.get("paint", {}).get("required", False):
        ...
        if paint_area > 0:
            quantities["primer_s"] = paint_area  # NO WASTE FACTOR (was 1.05)
            quantities["paint_s"] = paint_area   # NO WASTE FACTOR (was 1.05)

    if work_scope.get("insulation", {}).get("required", False):
        quantities["insulation_s"] = wall_area_gross * 1.05
        ...
        for key in required_keys:
            if key not in data:
                if key == "property_info":
                    ...
                    data[key] = {"content_manipulation_strategy": "", "floor_continuity_zones": [],
                        "paint_continuity_zones": []}
...
def generate_auto_justifications(room_data: Dict[str, Any], work_scope: Dict[str, Any]) -> Dict[str, str]:
    ...
    if work_scope.get("paint", {}).get("required", False):
        justifications["paint"] = ("IRC R702.3.8 requires proper surface preparation with PVA primer for adequate paint adhesion, "
                                   "no waste factor applied for paint coverage")
