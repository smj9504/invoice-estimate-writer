"""
Invoice service with comprehensive business logic and validation.
"""
...
from app.repositories import get_invoice_repository
...
class InvoiceService(TransactionalService[Dict[str, Any], str]):
    ...
    def update_with_items(self, invoice_id: str, invoice_data: Dict[str, Any]) -> Optional[Dict[str, Any]]:
        ...
        def _update_operation(session, invoice_id, data):
            ...
            # Update invoice
            updated_invoice = repository.update(invoice_id, data)
            if not updated_invoice:
                return None
            
            # Update items (delete existing and create new ones)
            # This is a simplified approach; a more sophisticated solution
            # would compare and update individual items
            if hasattr(repository, 'delete_items_by_invoice_id'):
                repository.delete_items_by_invoice_id(invoice_id)
