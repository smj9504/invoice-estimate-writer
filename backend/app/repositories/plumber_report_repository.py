...
from app.models.sqlalchemy_models import PlumberReport
...
class PlumberReportRepositoryMixin:
    ...
    def get_reports_by_status(self, status: str) -> List[Dict[str, Any]]:
        ...
        return self.get_all(filters={'status': status}, order_by='-report_date')
    
    def get_reports_by_company(self, company_id: str) -> List[Dict[str, Any]]:
        """Get plumber reports for a specific company"""
        return self.get_all(filters={'company_id': company_id}, order_by='-report_date')
    ...
    def get_reports_by_client(self, client_name: str) -> List[Dict[str, Any]]:
        ...
        return self.get_all(filters={'client_name': client_name}, order_by='-report_date')
    
    def get_recent_reports(self, limit: int = 10) -> List[Dict[str, Any]]:
        """Get recent plumber reports"""
        return self.get_all(order_by='-report_date', limit=limit)
...
class PlumberReportSQLAlchemyRepository(SQLAlchemyRepository, PlumberReportRepositoryMixin):
    ...
    def get_reports_by_date_range(self, 
        ...
        try:
            entities = self.db_session.query(PlumberReport).filter(
                PlumberReport.report_date >= start_date,
                PlumberReport.report_date <= end_date
            ).order_by(PlumberReport.report_date.desc()).all()
    ...
    def search_reports(self, search_term: str) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            entities = self.db_session.query(PlumberReport).filter(
                ...
                (PlumberReport.findings.ilike(search_pattern)) |
    ...
    def get_reports_by_water_source(self, water_source: str) -> List[Dict[str, Any]]:
        """Get plumber reports by water source"""
        try:
            entities = self.db_session.query(PlumberReport).filter(
                PlumberReport.water_source == water_source
            ).order_by(PlumberReport.report_date.desc()).all()
            
            return [self._convert_to_dict(entity) for entity in entities]
            
        except Exception as e:
            logger.error(f"Error getting plumber reports by water source: {e}")
            raise Exception(f"Failed to get plumber reports by water source: {e}")
    
    def get_reports_with_attachments(self) -> List[Dict[str, Any]]:
        """Get plumber reports that have attachments"""
        try:
            entities = self.db_session.query(PlumberReport).filter(
                PlumberReport.attachments.isnot(None)
            ).order_by(PlumberReport.report_date.desc()).all()
            ...
            for entity in entities:
                ...
                if report_dict.get('attachments') and isinstance(report_dict['attachments'], str):
                    try:
                        report_dict['attachments'] = json.loads(report_dict['attachments'])
                    except json.JSONDecodeError:
                        logger.warning(f"Invalid JSON in attachments for report {entity.id}")
                        report_dict['attachments'] = []
                
                # Only include reports with actual attachments
                if report_dict.get('attachments'):
                    reports.append(report_dict)
            
            return reports
            
        except Exception as e:
            logger.error(f"Error getting plumber reports with attachments: {e}")
            raise Exception(f"Failed to get plumber reports with attachments: {e}")
    
    def create(self, entity_data: Dict[str, Any]) -> Dict[str, Any]:
        """Create plumber report with JSON field handling"""
        # Handle JSON fields
        json_fields = ['inspection_areas', 'attachments']
        for field in json_fields:
            if field in entity_data and isinstance(entity_data[field], dict):
...
class PlumberReportSupabaseRepository(SupabaseRepository, PlumberReportRepositoryMixin):
        ...
        try:
            ...
            filtered_reports = [
                ...
                if (
                    ...
                    search_lower in (report.get('findings', '') or '').lower() or
    ...
    def get_reports_by_water_source(self, water_source: str) -> List[Dict[str, Any]]:
        """Get plumber reports by water source using Supabase"""
        try:
            response = self.client.table('plumber_reports').select('*').eq(
            ...
            logger.error(f"Error getting plumber reports by water source from Supabase: {e}")
            raise Exception(f"Failed to get plumber reports by water source: {e}")
    
    def get_reports_with_attachments(self) -> List[Dict[str, Any]]:
        """Get plumber reports that have attachments using Supabase"""
        try:
            # Get all reports and filter for those with attachments
            all_reports = self.get_all()
            ...
            for report in all_reports:
                ...
                if report.get('attachments'):
                    if isinstance(report['attachments'], str):
                        try:
                            attachments = json.loads(report['attachments'])
                            report['attachments'] = attachments
                        except json.JSONDecodeError:
                            logger.warning(f"Invalid JSON in attachments for report {report.get('id')}")
                            continue
                    
                    # Only include reports with actual attachments
                    if report['attachments']:
                        reports_with_attachments.append(report)
            
            # Sort by report date
            reports_with_attachments.sort(key=lambda x: x.get('report_date', ''), reverse=True)
            
            return reports_with_attachments
            
        except Exception as e:
            logger.error(f"Error getting plumber reports with attachments from Supabase: {e}")
            raise Exception(f"Failed to get plumber reports with attachments: {e}")
    ...
    def create(self, entity_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        json_fields = ['inspection_areas', 'attachments']
        for field in json_fields:
            if field in clean_data and isinstance(clean_data[field], dict):
            ...
            for field in json_fields:
                if report.get(field) and isinstance(report[field], str):
                    try:
                        ...
                        report[field] = {} if field == 'inspection_areas' else []
