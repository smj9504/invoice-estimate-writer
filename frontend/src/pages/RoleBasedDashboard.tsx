  ...
  return (
    <div style={{ padding: '24px' }}>
      ...
      <Row gutter={[16, 16]}>
        {/* Work Orders Section */}
        <Col xs={24} lg={16}>
          <Tabs defaultActiveKey="priority">
            ...
            <TabPane tab={`Overdue (${dashboardData.overdue_work_orders.length})`} key="overdue">
              {renderWorkOrderSection('Overdue Work Orders', dashboardData.overdue_work_orders, '#dc2626')}
            </TabPane>

            {dashboardData.documents_requiring_revision.length > 0 && (
              <TabPane tab={`Revisions (${dashboardData.documents_requiring_revision.length})`} key="revisions">
                <Card>
                  <Table
            ...
            {dashboardData.document_completions.map(stat => (
              <div key={stat.document_type} style={{ marginBottom: 16 }}>
                ...
                <Space style={{ width: '100%', justifyContent: 'space-between' }}>
                  ...
                  <Text type="secondary">${stat.total_amount.toLocaleString()}</Text>
                </Space>
              </div>
            ))}
          </Card>
